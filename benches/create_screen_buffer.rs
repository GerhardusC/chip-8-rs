...
fn create_buffer_string_iterator(c: &mut Criterion) {
    ...
    c.bench_function("Iterator Approach", |b| {
        b.iter(|| create_buffer_string(&vals, black_box(256)))
    });
}

fn create_buffer_string_push(c: &mut Criterion) {
    let vals: [u8; 0xFFFF] = [0; 0xFFFF];
    c.bench_function("Single Allocation Approach", |b| {
        b.iter(|| create_one_alloc_buffer_string(&vals, black_box(256)))
    });
}

criterion_group!(
    benches,
    create_buffer_string_iterator,
    create_buffer_string_push
);
